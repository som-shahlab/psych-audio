...
def main(args):
    ...
    print(f"Encoding...")
    random_embeddings = eeu.batch_encode(
        args.embedding_name, model, keys, random_sentences
    )
    corpus_embeddings = eeu.batch_encode(
        args.embedding_name, model, keys, corpus_sentences
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--embedding_name", default="word2vec", choices=["word2vec"]
    )
    parser.add_argument(
        "--n",
        type=int,
