...
def main(args):
    ...
    if has_file:
        ...
        axes[0].hist(
            cosine, 100, density=True, facecolor="g", alpha=0.75, label="cosine"
        )
        axes[0].hist(
            euclidean,
            100,
...
def generate_embeddings(args, bc=None):
    ...
    if args.embedding_name in ["word2vec", "glove"]:
        embeddings = np.zeros(
            (args.n, config.F[args.embedding_name]), np.float32
        )
        for i, s in enumerate(sentences):
            embed = evaluation.embeddings.util.encode_from_dict(
                args.embedding_name, model, keys, s
            )
            if embed is None:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "n", type=int, default=100, help="Number of sentences to generate."
    )
    parser.add_argument(
        "embedding_name", type=str, choices=["word2vec", "glove", "bert"]
    )
