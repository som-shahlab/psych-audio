...
def main(args):
    ...
    model, keys = evaluation.embeddings.util.load_embedding_model(
        args.embedding_name
    )
    ...
    for i in tqdm(range(len(paired_gids)), desc="Embedding"):
        ...
        embed_gt = evaluation.embeddings.util.encode_from_dict(
            args.embedding_name, model, keys, paired[gid]["gt"]
        )
        embed_pred = evaluation.embeddings.util.encode_from_dict(
            args.embedding_name, model, keys, paired[gid]["pred"]
        )
    ...
    # Convert to numpy.
    pred_sentences, gt_sentences = (
        np.asarray(sentences["pred"]),
        np.asarray(sentences["gt"]),
    )
    pred_embeddings, gt_embeddings = (
        np.asarray(embeddings["pred"]),
        np.asarray(embeddings["gt"]),
    )
    pred_gids, gt_gids = (
        np.asarray(out_gids["pred"]),
        np.asarray(out_gids["gt"]),
    )
    ...
    np.savez_compressed(
        gt_fqn, embeddings=gt_embeddings, gids=gt_gids, text=gt_sentences
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "embedding_name", type=str, choices=["word2vec", "glove"]
    )
