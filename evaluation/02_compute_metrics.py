...
def compute_metrics(args, paired: Dict) -> Dict:
    ...
    for _ in range(args.n_threads):
        t = Thread(
            target=worker, args=(args, q, r, model, w2v_keys, gts, preds)
        )
        threads.append(t)
...
def worker(
    args, q: Queue, r: Queue, model: Dict, w2v_keys: Set, gts: Dict, preds: Dict
):
...
def get_mean_std(
    speaker: str, metric_name: str, accumulator: Dict, dim: str, val: int
):
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--n_threads", default=4, type=int, help="Number of threads to use."
    )
    parser.add_argument(
        "--no_embedding",
        action="store_true",
