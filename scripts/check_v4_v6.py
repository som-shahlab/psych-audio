...
def main(args):
	...
	for filename in tqdm(unique, desc='Comparing Files'):
		...
		if hash4 != hash6:
			...
			source_fqn = os.path.join(jasa_dir, 'v4', 'gt', transcribed_filename)
			target_fqn = os.path.join(jasa_dir, 'v6', 'gt', transcribed_filename)
			shutil.copy(source_fqn, target_fqn)

			source_fqn = os.path.join(jasa_dir, 'v4', 'machine-video', transcribed_filename)
			target_fqn = os.path.join(jasa_dir, 'v6', 'machine-video', transcribed_filename)
			shutil.copy(source_fqn, target_fqn)
