...
def main(args):
    ...
    completed_hashes = get_hashes_from_dir(
        os.path.join(jasa_dir, "v5", "machine-video")
    )

    # Select the next N files.
    untranscribed_hashes = list(
        set(all_hashes).difference(set(completed_hashes))
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "n", type=int, help="Number of files to queue for upload."
    )
